...
const MyReserve = ({onCancel}) => {
  ...
  const activeOnlybooking = myReserve.filter(item => item.status === 'active');
                ...
                activeOnlybooking.map((item)=>{
                  return (
                    <li key={item.id}>
                      ...
                      <p className="now-date">{item.start_date}<br /> ~ {item.end_date} </p>
                      <p className="now-space"><TbParkingCircle />{item.space_id}</p>  {/*주차 위치 표시*/}
                      {
                      item.status === 'active' ? (
                      <button
                        onClick={()=>{navigator('/cancelcomplete')
                        onCancel({
                          bookingId : item.id,
              ...
              notactivebooking.map((item)=>{
                return (
                  <li key={item.id}>
                    ...
                    <p className="end-date">{item.start_date}<br /> ~ {item.end_date}</p>
                    <p className="end-space"><TbParkingCircle />{item.space_id}</p>  {/*주차 위치 표시*/}
